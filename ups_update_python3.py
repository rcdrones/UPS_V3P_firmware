...
def UART_TRANSFER_AUTO(thelist, PN):
    global com
    # thelist is buffer[0 to 63]
    #PN = package number , unsigned int 32bit
    thelist[4] = PN & 0xff
    thelist[5] = PN >> 8 & 0xff
    thelist[6] = PN >> 16 & 0xff
    thelist[7] = PN >> 24 & 0xff
    ...
    try:
        while (True):
            ...
            if(len(return_buffer) != 0):
                ...
                packege_checksum = (return_buffer[1] << 8) | packege_checksum
                if checksum != packege_checksum:
                    print("RX date detected, program auto link again.")
                else:
                    print("Link UPS success.")
    #                 # error_return()
                RPN = 0
                RPN = return_buffer[4]
                RPN = (return_buffer[5] << 8) | RPN
        ...
        print("Find uart date error, run command again.")
        com.close()
        sys.exit()
...
def READ_FW_FUN():
    ...
    print("FW_VERSION = 0x%x" % FW_VERSION)
...
def READ_APROM_BIN_FILE(FILENAME):
    ...
    try:
        ...
        print("Firmware File load error")
        print (len(AP_FILE))
        print(e)
        error_return()
    ...
    for i in range(48, AP_SIZE, 56):
        ...
        if (((AP_SIZE - i) < 56) or ((AP_SIZE - i) == 56)):
            ...
            if(d_checksum == (AP_CHECKSUM & 0xffff)):
                print("Update firmware successful.")
        else:
            # print "loop"
...
if len(sys.argv) != 3:
    print("python3 ups_update_python3.py port_number file_name")
    print("example: python3 ups_update_python3.py /dev/ttyAMA0 app.bin")
